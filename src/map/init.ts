/**
 * Karten-Modul: wird per dynamic import nachgeladen, damit das schwere
 * MapLibre-Bundle den ersten Seitenaufbau (Panel, Hero) nicht blockiert.
 */
import maplibregl from "maplibre-gl";
import "maplibre-gl/dist/maplibre-gl.css";
import type { Airport } from "../logic/types.ts";
import { loadBasemapStyle } from "./basemap.ts";
import {
  addAtlasLayers,
  updateAirports,
  updateRings,
  ...
  map.on("click", (e) => {
    ...
    if (s.routePicking && s.routeA) {
      ...
      map.fitBounds(
        [
          [
            Math.min(s.routeA.lo, best.lo) - 3,
            Math.max(28, Math.min(s.routeA.la, best.la) - 7),
          ],
          [
            Math.max(s.routeA.lo, best.lo) + 3,
            Math.min(71, Math.max(s.routeA.la, best.la) + 2),
          ],
        ],
        { padding: 40 },
      );
